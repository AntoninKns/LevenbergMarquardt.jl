export levenberg_marquardt_GPUAD, levenberg_marquardt_GPUAD!
...
"""
function levenberg_marquardt_GPUAD(model; kwargs...)
  solver = LMSolverGPUAD(model)
  levenberg_marquardt_GPUAD!(solver, model; kwargs...)
  return solver.stats
end
...
LARGE SPARSE NONLINEAR LEAST SQUARES" from Wright and Holt
"""
function levenberg_marquardt_GPUAD!(solver    :: AbstractLMSolver{T,S,ST},
  ...
  GPUx, GPUFx, GPUFxp, GPUFxm, GPUd = solver.GPUx, solver.GPUFx, solver.GPUFxp, solver.GPUFxm, solver.GPUd
  GPUJv, GPUJtv, GPUJu, GPUJtu = solver.GPUJv, solver.GPUJtv, solver.GPUJu, solver.GPUJtu

  # Set up the initial value of the residual and Jacobian at the starting point
  residualGPU!(model, x, Fx, GPUFx)

  GPUJx = jac_op_residual!(model, GPUx, GPUJv, GPUJtv)
